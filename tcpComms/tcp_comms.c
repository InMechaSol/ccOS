...
void serverListenAccept(struct tcpServerStruct* tcpServerStructPtrIn)
{
    switch (tcpServerStructPtrIn->tcpData.tcpStatus)
     ...
     case tcpstat_connecting: // listen/accept being called cyclically until client connection
         ...
         if (tcpServerStructPtrIn->tcpData.iResult == SOCKET_ERROR)
         {
             if (tcpServerStructPtrIn->tcpData.lastErr != WSAEWOULDBLOCK && tcpServerStructPtrIn->tcpData.lastErr != EAGAIN)
...
void shutdownRestartClientSocket(struct tcpStruct* tcpStructPtrIn)
{
    switch (tcpStructPtrIn->tcpStatus)
     ...
     case tcpstat_connecting:
         ...
         if (tcpStructPtrIn->lastErr == 0 || tcpStructPtrIn->lastErr == WSAEWOULDBLOCK || tcpStructPtrIn->lastErr == WSAEALREADY || tcpStructPtrIn->lastErr == EAGAIN || tcpStructPtrIn->lastErr == EINPROGRESS)
         {
             ...
             if (tcpStructPtrIn->iResult == SOCKET_ERROR)
             {
                 ...
                 if (tcpStructPtrIn->lastErr != WSAEWOULDBLOCK && tcpStructPtrIn->lastErr != WSAEISCONN && tcpStructPtrIn->lastErr != WSAEALREADY && tcpStructPtrIn->lastErr != EAGAIN && tcpStructPtrIn->lastErr != EINPROGRESS)
